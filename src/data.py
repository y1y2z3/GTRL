...
class DistData(data.Dataset):
    def __init__(self, path, dataset, num_nodes, num_rels, set_name):
        quadrupleList, times, triples = utils.load_quadruples(path + dataset, set_name + '.txt')
        true_prob_s, true_prob_r, true_prob_o = utils.get_true_distributions(path, times, triples, num_nodes, num_rels,
                                                                             dataset, set_name)
...
class EntDistGivenTRData(data.Dataset):
    def __init__(self, path, dataset, num_nodes, num_rels, set_name, seq_len, num_r=None):
        if num_r:
            t_data, r_data, r_hist, r_hist_t, true_prob_s, true_prob_o = utils.get_scaled_tr_dataset(num_nodes, path,
                                                                                                     dataset, set_name,
                                                                                                     seq_len, num_r)
        else:
    ...
    def __getitem__(self, index):
        return self.t_data[index], self.r_data[index], self.r_hist[index], self.r_hist_t[index], self.true_prob_s[
            index], self.true_prob_o[index]
